# Golem Provider Autoconfig
    ...
    #Date: 29 March 2021
    #Version: 0.1
read -p "Welcome to the Golem Provider installer! This program will automatically download the provider and start it for  you plus give you the option to do some other cool stuff. Press ENTER when you're ready to start."
...
echo "Do you wish to install Netdata, a really cool system monitoring tool?"
select yn in "Yes" "No"; do
    case $yn in
        Yes ) bash <(curl -Ss https://my-netdata.io/kickstart.sh); break;;
        No ) echo "That's OK! Let's continue."; break;;
    esac
done
...
select yn in "Yes" "No"; do
    case $yn in
        Yes ) firefox http://localhost:19999; disown; break;;
        No ) echo "That's OK! You can install it anytime by running bash <(curl -Ss https://my-netdata.io/kickstart.sh"; break;;
    esac
done
echo "Are you ready to monitor Golem?"
select yn in "Yes" "No"; do
    case $yn in
        Yes ) watch -c golemsp status; break;;
        No ) echo "No problem! You can monitor the Golem provider by running 'golemsp status' anytime from the terminal, or for continuous monitoring, try 'watch -c golemsp status'. Installation is now complete. Thanks for being a part of the Golem Provider Network!"; break;;
    esac
done
